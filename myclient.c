...
int main(int argc, char **argv)
{
   ...
   do {
    printf("Enter a command (SEND, LIST, READ, DEL, QUIT) or type 'quit' to exit>> ");
    if (fgets(buffer, BUF, stdin) != NULL) {
        // ... [Rest of the original buffer processing]
        ...
        if (size == -1) {
           ...
           buffer[size] = '\0';
            if (strcmp(buffer, "OK") == 0) {
                printf("Command processed successfully by the server.\n");
            } else {
